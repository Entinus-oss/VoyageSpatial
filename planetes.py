...
class Planet():
    ...
    def calcOrbit(self, time):
        ...
        self.orbit = np.array(self.orbit)

        return np.array(self.orbit)

    def orbitalSpeed(self, time):
        vDir = [- self.axe * self.omega * np.sin(self.omega * time[i] + phase), self.b * self.omega * np.cos(self.omega * time[i] + phase)]
        vDir = vDir / np.linalg.norm(vDir)

        for i in range(time.size):
            r = np.linalg.norm(self.orbit[time[i]])
            v = np.sqrt(G * self.mass * (2/r - 1/self.axe))
            self.orbitSpeed.append(v * vDir)   
        self.orbitSpeed = np.array(self.orbitSpeed)
